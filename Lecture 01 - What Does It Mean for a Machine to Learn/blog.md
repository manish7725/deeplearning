...
Both refuse to report zero. Both do the job.

### Why does a smooth error function help us learn?

Before we talk about calculus, imagine something physical.

Our model has **dials** — for example, $w$ and $b$. Turning those dials changes the predictions, which changes the loss.

Now imagine that loss is the **height of a hill**, and our current model is a little person standing somewhere on that hill.

Our goal is simple:

> **Get the person to the bottom of the hill, because the bottom means the error is as small as possible.**

But the person is blind. They cannot see where the bottom is.

So they need one useful clue:

> **Which way is downhill?**

That clue is the **slope**.

### A smooth hill gives a smooth clue

Imagine walking down a smooth hill.
...
As you get closer to the bottom, the hill becomes less steep.

So the slope gives a very useful signal:

- Far from the bottom → the slope can be large, so take a stronger step.
- Closer to the bottom → the slope becomes smaller, so take a smaller step.
- At the bottom → the slope is zero, so stop.

The information changes **gradually**.

That is exactly what a learning algorithm likes.

### What happens with a sharp corner?

Now imagine a roof with a sharp peak:
...
If you stand just to the left, downhill points one way.

If you stand just to the right, downhill points the other way.

But **exactly at the sharp corner**, there is no single, well-defined slope.

The direction changes abruptly.

This is what happens with the absolute error $|e|$ at $e=0$.
...
The point at zero is a corner, not a smooth bottom.

### Why do we care?

Because from later chapters onward, our learning method repeatedly asks:

> **“If I turn this dial a tiny bit, will the loss go up or down?”**

For a smooth loss function, that question has a clear answer at every point.

For a function with a sharp corner, there are points where the usual slope is not defined.

So the key idea is **not**:

> “Sharp functions are bad.”

It is:

> **For the slope-based learning method we are about to build, a smooth loss function gives us a clear direction for changing the model parameters.**

### Why squared error is convenient

Now look at squared error:

$$
L = e^2
$$

Its graph is a smooth bowl:
...
There is no sudden corner.

As the error gets closer to zero, the slope changes gradually toward zero.

So our learning algorithm can keep asking the same question everywhere:

> **Which way should I turn the dial, and how strongly?**

That is the real reason smoothness is useful here.

> 💡 **For now, remember this chain:**
>
> **Smooth loss → clear slope → clear direction → easier parameter updates → easier learning.**

Later, calculus will give this idea a precise name: **differentiability**. We do not need that word yet.

### So why do we still prefer the square?

The smoothness reason is important, but it is not the only reason.
...
2. **It is smooth.** Unlike $|e|$, $e^2$ has a well-defined slope everywhere, including at zero.
